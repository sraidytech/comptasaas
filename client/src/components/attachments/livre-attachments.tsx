...
interface LivreAttachmentsProps {
  livreId: string;
}

export function LivreAttachments({ livreId }: LivreAttachmentsProps) {
  const [attachments, setAttachments] = useState<Attachment[]>([]);
  const { execute: fetchAttachments, loading, error } = useAsync<Attachment[]>(
    () => attachmentsApi.getLivreAttachments(livreId)
  );
  ...
  useEffect(() => {
    // Fetch attachments whenever livreId changes or component mounts
    // But only if we're not already fetching
    if (livreId && !isFetchingRef.current) {
    ...
    <div className="mt-6">
      {loading ? (
        ...
        <AttachmentList
          ...
          title="Pièces jointes du livre"
          emptyMessage="Aucune pièce jointe n'a été téléchargée pour ce livre"
        />
      )}
    </div>
  );
